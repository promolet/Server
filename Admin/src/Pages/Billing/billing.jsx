...
import AddAddress from "./AddressAdd";
const Billingpage = () => {
  ...
  // Calculate cartTotal based on whether it's a single product or multiple products
const cartTotal = location.state?.type === "single" 
...
: cart?.reduce(
    ...
    0
  ) || 0; // Default to 0 if cart is empty or undefined

// Calculate total amount including shipping, tax, points, and wallet balance
const totalAmount = location.state?.type === "single" 
...
: cartTotal + shippingCost + taxAmount - pointsBalance - walletBalance; // Multiple products case
    ...
    const handlePlaceOrder = async () => {
      ...
      const addressResponse = await axios.get(`https://api.prumolet.com/api/addresses/${userId}/${selectedAddressId}`);
      ...
      const orderData = {
        ...
        product: location.state?.type === "single" 
          ? [
              {
    ...
    try {
      if (location.state?.type === "single") {
        ...
        if (response.data.cart && Array.isArray(response.data.cart.products)) {
          ...
          const productsWithDetails = await fetchProductDetails(response.data.cart.products);
  ...
  fetchDetails();
}, [location.state],[id]); // Dependency on location.state so the effect reruns on change
                                ...
                                addresses.map((address) => (
                                  <li key={address._id}>
                                    ...
                                    <label
                                      ...
                                      for="check3"
                                    >
                                      <span class="name">{address.title}</span>
                                ...
                                <div className="col-xxl-6 col-lg-12 col-md-6">
                                  <div className="delivery-address-box">
                                    ...
                                    <label
                                      ...
                                      htmlFor="check7"
                                    >
                                      Standard Delivery | Approx 5 to 7 Days
                                    </label>
                                  </div>
                                </div>
  ...
  <div className="cart-items">
    ...
    <div className="cart-content">
      ...
      <span className="text-theme">${(product?.price || 0).toFixed(2)}</span>
  ...
  {location.state?.type === "single" ? (
    ...
    <ul className="sub-total">
      <li>
        Sub Total <span className="count">₹{product?.price.toFixed(2)}</span>
      </li>
      <li>
        <h4>Wallet Balance</h4>
        <h4 className="price">₹{walletBalance.toFixed(2)}</h4>
      </li>
      <li className="border-cls">
        <label for="wallet" className="form-check-label m-0">
    ...
    <ul className="sub-total">
      <li>
        Sub Total <span className="count">₹{cartTotal.toFixed(2)}</span>
      </li>
      <li>
        Shipping <span className="count">₹{shippingCost.toFixed(2)}</span>
      </li>
      <li>
        Tax <span className="count">₹{taxAmount.toFixed(2)}</span>
      </li>
      <li>
        <h4 className="txt-muted">Points</h4>
        <h4 className="price txt-muted">₹{pointsBalance.toFixed(2)}</h4>
      </li>
      <li className="border-cls">
        <label for="points" className="form-check-label m-0">
...
<ul className="total">
  {location.state?.type === "single" ? (
    <li>
      Total <span className="count">₹{product?.price.toFixed(2)}</span>
    </li>
  ) : (
    <li>
