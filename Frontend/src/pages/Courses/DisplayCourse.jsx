...
const CourseDetail = () => {
  ...
  useEffect(() => {
    const fetchProductData = async () => {
      try {
        const response = await axios.get(`http://193.203.162.54:5500/courses/${id}`);
        ...
        if (userId) {
          const purchaseResponse = await axios.get(
            `http://193.203.162.54:5500/api/course/purchase-status?userId=${userId}&courseId=${id}`
          );
  ...
  const handleBuyNow = async () => {
    const userId = localStorage.getItem("userId");
    try {
      const response = await axios.post("http://193.203.162.54:5500/create-order", {
      ...
      const options = {
        ...
        handler: async function (response) {
          await axios.post("http://193.203.162.54:5500/api/course/data", {
          ...
          <div className="row">
            <div className="col-lg-6">
              <iframe
                ...
                width="100%"
                height="315"
                src={`http://193.203.162.54:5500${product.videos[0]}`}
                title="Course Video"
                frameBorder="0"
                allowFullScreen
              ></iframe>
                ...
                {product.videos.map((video, index) => (
                  <li key={index}>
                    <iframe
                      ...
                      width="100%"
                      height="200"
                      src={`http://193.203.162.54:5500${video}`}
                      frameBorder="0"
                      allowFullScreen
                    ></iframe>
                  </li>
